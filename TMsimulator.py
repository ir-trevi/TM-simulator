...
class Interface:
    ...
    def show(self) -> None:
        ...
        if self.code_size + tape_size * (2 if slim_tape else 4) + 10 > length or height < 20 or length < 120:
        ...
        for i in range(height):
            if i == 0:
                ...
                buffer_string += f"╠{'═' * (length - self.code_size - 2)}╤{'═' * (self.code_size - 1)}╣" if not no_code \
                            else f"╠{'═' * (length - 2)}╣"
            elif i == c - 6:
                upper_part = f"┌{'─' * (tape_size - 2)}┐"
                buffer_string += (f"║{upper_part.center(length - self.code_size - 2)}│" + show_code(i, False)) if not no_code \
                            else f"║{upper_part.center(length - 2)}║"
            elif i == c - 5:
                text = f"│{self.state.center(tape_size - 2)}│"
                buffer_string += (f"║{text.center(length - self.code_size - 2)}│" + show_code(i, False)) if not no_code \
                            else f"║{text.center(length - 2)}║"
            elif i == c - 4:
                lower_part = f"└{'─' * (tape_size - 2)}┘"
                buffer_string += (f"║{lower_part.center(length - self.code_size - 2)}│" + show_code(i, False)) if not no_code \
                            else f"║{lower_part.center(length - 2)}║"
            elif i == c + 2:
                buffer_string += (f"║{'│'.center(length - self.code_size - 2)}│" + show_code(i, False)) if not no_code \
                            else f"║{'│'.center(length - 2)}║"
            elif i == c + 3:
                arrow_tip = '*' if self.writing else 'V'
                buffer_string += (f"║{arrow_tip.center(length - self.code_size - 2)}│" + show_code(i, True)) if not no_code \
                            else f"║{arrow_tip.center(length - 2)}║"
            elif i == c + 4:
                upper_tape = f"─{('┬─' if slim_tape else '─┬──') * (tape_size + 1)}"
                buffer_string += (f"║{upper_tape.center(length - self.code_size - 2)}│" + show_code(i, False)) if not no_code \
                ...
                buffer_string += (f"║{tape_content.center(length - self.code_size - 2)}│" + show_code(i, False)) if not no_code \
                            else f"║{tape_content.center(length - 2)}║"
            elif i == c + 6:
                lower_tape = f"─{('┴─' if slim_tape else '─┴──') * (tape_size + 1)}"
                buffer_string += (f"║{lower_tape.center(length - self.code_size - 2)}│" + show_code(i, False)) if not no_code \
                            else f"║{lower_tape.center(length - 2)}║"
            elif i == height - 5:
                buffer_string += (f"╟{'─' * (length - self.code_size - 2)}┤" + show_code(i, False)) if not no_code \
                            else f"╟{'─' * (length - 2)}╢"
            elif i == height - 4:
                input_string = self.input_tape.upper()
                buffer_string += (f"║  Input: {input_string.ljust(length - self.code_size - 10)}"[:length - self.code_size - 1] +
...
def main():
    ...
    if speed < 1 or speed > 10:
        ...
        exit()
    if not (auto or (code_size or tape_size) or instant):
    ...
    if not instant and not debug:
        ...
        if not auto:
            ...
            if length > 172:
                slim_tape = False
                tape_size = 33
                code_size = length - tape_size * 4 - 14 if code_size != 0 else 0
            else:
                slim_tape = True
                tape_size = 33
